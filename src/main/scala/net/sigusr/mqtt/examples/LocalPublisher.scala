...
class LocalPublisher(toPublish: Vector[String]) extends Actor {
  ...
  context.actorOf(Manager.props(new InetSocketAddress("ubuntu",1883))) ! Connect(localPublisher)/////////////////////////////////////

  val length = toPublish.length

  def scheduleRandomMessage(): Unit = {
    val message = toPublish(scala.util.Random.nextInt(length))
    context.system.scheduler.scheduleOnce(FiniteDuration(scala.util.Random.nextInt(2000).toLong + 1000, MILLISECONDS), self, message)
    //val message = "gggg"
    //context.system.scheduler.scheduleOnce(FiniteDuration(5000.toLong, MILLISECONDS), self, message)
    ()
  }
